...
function App() {
  ...
  const addItem = (sectionId: string, subsectionTitle?: string) => {
    setCanvas(prev => prev.map(section => {
      if (section.id === sectionId) {
        ...
        } else if (!subsectionTitle && section.items.length < 3) {
  ...
  const removeItem = (sectionId: string, index: number, subsectionTitle?: string) => {
    setCanvas(prev => prev.map(section => {
      if (section.id === sectionId) {
        if (subsectionTitle && section.subsections) {
          return {
  ...
  const updateItem = (sectionId: string, index: number, value: string, subsectionTitle?: string) => {
    setCanvas(prev => prev.map(section => {
      if (section.id === sectionId) {
        if (subsectionTitle && section.subsections) {
          return {
            ...
            subsections: section.subsections.map(sub => 
              sub.title === subsectionTitle
                ? { ...sub, items: sub.items.map((item, i) => i === index ? value : item) }
                : sub
            )
          };
        } else if (!subsectionTitle) {
          return { ...section, items: section.items.map((item, i) => i === index ? value : item) };
        }
      }
      return section;
    }));
  ...
  const renderItems = (items: string[], sectionId: string, subsectionTitle?: string) => (
      ...
      {items.map((item, index) => (
        <div key={index} className="flex items-center gap-2 group">
          <input
            type="text"
            value={item}
            onChange={(e) => updateItem(sectionId, index, e.target.value, subsectionTitle)}
  ...
  const renderSection = (section: CanvasSection, className: string = "") => (
    <div className={`border-2 border-gray-300 p-3 h-full flex flex-col relative ${className || 'bg-white'}`}>
      ...
      {!section.subsections && (
        <div className="flex items-center justify-between mb-2">
          <h3 className="font-bold text-sm text-gray-900 uppercase tracking-wide">{section.title}</h3>
        ...
        {section.subsections ? (
          <div className="h-full flex flex-col">
            {section.subsections.map((subsection) => (
              <div key={subsection.title} className="flex-1 border-b border-gray-200 pb-2 last:border-b-0 last:pb-0 flex flex-col">
                <div className="flex items-center justify-between mb-2">
                  <h3 className="font-bold text-sm text-gray-900 uppercase tracking-wide">{subsection.title}</h3>
                  <div className="text-xs text-gray-500 bg-gray-100 px-2 py-1 rounded">
  ...
  const getSectionById = (id: string) => canvas.find(section => section.id === id)!;

  return (
    <div className="min-h-screen bg-gray-100 py-6">
      <div className="max-w-7xl mx-auto px-4">
        <div className="text-center mb-6">
          <h1 className="text-3xl font-bold text-gray-900 mb-2">Lean Canvas Editor</h1>
        ...
        <div className="bg-white p-4 rounded-lg shadow-lg">
          <div className="grid grid-cols-10 grid-rows-3 gap-0 h-[600px]">
            ...
            <div className="col-span-2 row-span-2">
              {renderSection(getSectionById('unique-value-proposition'), 'bg-yellow-50')}
            </div>
            <div className="col-span-2">
              {renderSection(getSectionById('unfair-advantage'), 'bg-purple-50')}
            </div>
            <div className="col-span-2 row-span-2">
              {renderSection(getSectionById('customer-segments'), 'bg-indigo-50')}
            </div>
